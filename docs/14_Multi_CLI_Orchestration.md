...
All CLIs share the same identity but through different config files:

| CLI | Config File | Identity Depth |
|-----|-----------|---------------|
| Primary | `CLAUDE.md` | Full (SOUL + memory + rules + dispatch) |
| Overflow | Agent definition only | Minimal (language rules + output format) |
| Reviewer | `AGENTS.md` | Read-only (security rules + review checklist) |
